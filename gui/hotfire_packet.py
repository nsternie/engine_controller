...
class ECParse:
	...
	def __init__(self):
		...
		self.items[97] = 'e28v' 
		self.items[98] = 'e5v' 
		self.items[99] = 'BOARD_ID' 
		self.items[100] = 'last_packet_number' 
		self.items[101] = 'last_command_id' 
		self.items[102] = 'tbrd' 
		self.items[103] = 'tvlv' 
		self.items[104] = 'tmtr' 
		self.items[105] = 'error_code' 
		self.items[106] = 'LOGGING_ACTIVE' 
		self.items[107] = 'current_page' 
		self.items[108] = 'zero' 
		self.items[109] = 'zero' 
		self.items[110] = 'zero' 
		self.items[111] = 'zero' 
	...
	def parse_packet(self, packet):
		...
		self.pressure[8] = float((float(struct.unpack("<h", byte_rep)[0]))/2)
		self.dict[self.items[9]] = self.pressure[8]
		byte_rep = packet[22:24]
		self.pressure[9] = float((float(struct.unpack("<h", byte_rep)[0]))/3)
		self.dict[self.items[10]] = self.pressure[9]
		byte_rep = packet[24:26]
		self.pressure[10] = float((float(struct.unpack("<h", byte_rep)[0]))/4)
		self.dict[self.items[11]] = self.pressure[10]
		byte_rep = packet[26:28]
		self.pressure[11] = float((float(struct.unpack("<h", byte_rep)[0]))/5)
		self.dict[self.items[12]] = self.pressure[11]
		byte_rep = packet[28:30]
		self.pressure[12] = float((float(struct.unpack("<h", byte_rep)[0]))/6)
		self.dict[self.items[13]] = self.pressure[12]
		byte_rep = packet[30:32]
		self.pressure[13] = float((float(struct.unpack("<h", byte_rep)[0]))/7)
		self.dict[self.items[14]] = self.pressure[13]
		byte_rep = packet[32:34]
		self.pressure[14] = float((float(struct.unpack("<h", byte_rep)[0]))/8)
		self.dict[self.items[15]] = self.pressure[14]
		byte_rep = packet[34:36]
		self.pressure[15] = float((float(struct.unpack("<h", byte_rep)[0]))/9)
		self.dict[self.items[16]] = self.pressure[15]
		byte_rep = packet[36:38]
		self.pressure[16] = float((float(struct.unpack("<h", byte_rep)[0]))/10)
		self.dict[self.items[17]] = self.pressure[16]
		byte_rep = packet[38:40]
		self.pressure[17] = float((float(struct.unpack("<h", byte_rep)[0]))/11)
		self.dict[self.items[18]] = self.pressure[17]
		byte_rep = packet[40:42]
		self.pressure[18] = float((float(struct.unpack("<h", byte_rep)[0]))/12)
		self.dict[self.items[19]] = self.pressure[18]
		byte_rep = packet[42:44]
		self.pressure[19] = float((float(struct.unpack("<h", byte_rep)[0]))/13)
		self.dict[self.items[20]] = self.pressure[19]
		byte_rep = packet[44:46]
		self.pressure[20] = float((float(struct.unpack("<h", byte_rep)[0]))/14)
		self.dict[self.items[21]] = self.pressure[20]
		byte_rep = packet[46:48]
		self.pressure[21] = float((float(struct.unpack("<h", byte_rep)[0]))/15)
